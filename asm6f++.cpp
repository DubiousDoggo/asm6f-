  ...
  for (int i = 1; i < argc; i++)
  {
    if (*argv[i] == '-')
    {
      ...
      case 'd':
        if (argv[i][2])
        {
          if (getlabel(my_strdup(&argv[i][2])) == nullptr)
          {
...
void processline(const std::string &src, const std::string &errsrc, const unsigned errline)
{
  ...
  { // not in a macro or rept definition
    ...
    { // not a reserved word, maybe a label?
      std::string word;
      if (goodlabel(word, line_iter_temp, line_end))
...
int eval(std::string::const_iterator &str, const std::string::const_iterator &end, const precedence_t precedence_t)
{
  ...
  } while (prec[op] > precedence_t && !errmsg);
  return ret;
}
...
void addlabel(std::string label_name, bool force_local)
{
  ...
  label *p = getlabel(label_name); // check if theres an existing label with the same name
...
void opcode(label *id, std::string::const_iterator &next, const std::string::const_iterator &end)
{
  ...
  { // loop through all allowed addressing modes for this instruction
    ...
    dependant = false;
    errmsg = nullptr;
    const operand_t type = mode.first;
...
void ifdef(label *id, std::string::const_iterator &next, const std::string::const_iterator &end)
{
  if (iflevel >= IFNESTS - 1)
    ...
    iflevel++;
  std::string temp;
  goodlabel(temp, next, end);
  skipline[iflevel] = (getlabel(temp) == nullptr) || skipline[iflevel - 1];
